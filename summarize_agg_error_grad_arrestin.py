...
geno_find = re.compile('ARR|DRWT', re.IGNORECASE)
...
def procPath(path):
    ...
    return [date.lower(), mouse.lower(), ear.lower(), eye.lower(), treatment.lower(), geno.lower()]
...
def findOCT(top_path):
    print(top_path)
    info_labels = ["date", "mouse", "ear", "eye", "treatment", "geno","Ymin","Ymax","PR Thickness (um)","IPL Thickness (um)","Midpoint Scatter","PR Scatter","Median PR Scatter","IPL Scatter","Median IPL Scatter","Ratio PR/IPL", "Median Scatter Ratio", "Midpoint:IPL Average Scatter"]
