...
def sorting(top_path):
    ...
    for folder in amp_folders:
        ...
        try:
            stack_pth = folder.parents[2] / 'stacks'
            stack_pth.mkdir()
        ...
        #split
        #current split will be based on 32 volumes, but this will be an important parameter to keep in mind!
        for ind in range(32):
            #whole stack has shape (total b-scans, z, x)
            #need every 32nd slice
            #start on the ith slice and take slices that are multiples of 32 away
            new_stack = stack[ind::32]
            save_path = stack_pth / 'stack{}.tif'.format(ind+vol_ind)
            io.imsave(str(save_path), new_stack)
        vol_ind+=32  
