...
def find_amp(top_path, args):
    ...
    #flag to test if generator is empty
    proc_flag=0
    path = top_path+os.sep+'**'+os.sep+'Log_Amp_FFT2X'
    #This is the generator. It finds all the paths of the dispersion compensated OCT data.
    amp_folders = glob.iglob(path, recursive=True)
    for folder in amp_folders:
        #raise flag if the generator is NOT empty
        proc_flag=1
        if 'processed_OCT_images' in folder:
            continue
        else:
            #2019-9-17 Tries to ignore empty Log_Amp_FFT2X folders. Not well tested.
            try:
                print('\n*****\nProcessing:\t{}\n*****\n'.format(folder))
                ...
                print("Empty Log_Amp_FFT2X folder. Dispersion Compensation may not have been run.")
                continue
    if proc_flag == 0:
        print('\n**The program ran without processing any data because it found no Log_Amp_FFT2X folders.**\
              \n**You may be in the incorrect folder (or datadrive!) or forgot to do dispersion compensation.**\n')
        sys.exit('\nExited: No Dispersion Compensation folders were found.')    
